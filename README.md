...
游戏包含**开始界面**、**关卡选择**、**游戏界面**和**通关 / 失败界面**。游戏界面显示
当前关卡、棋盘、剩余箭头数量、剩余失误次数、本关用时与累计得分，以及"重新开始"
按钮和底部四个工具按钮。

鼠标移到箭头上时，该箭头会**放大并亮起蓝色光晕**，光标同时变成手型，提示"这一格
可以点"；配合橙色的"提示"高亮，两者一眼可分辨：
...
其中悬停反馈除状态断言外，还有一个**像素级回归测试**：把悬停前后的画面各渲染一遍，
逐格比较像素，要求悬停格必须变化、其余格必须不变。这样"高亮没画出来"或
"高亮糊到隔壁格子"都能被测出来。
